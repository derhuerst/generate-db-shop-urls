...
const nextStepLink = (query, $, row) => {
	...
	if (u.query.seqnr) u.query.seqnr = '' + (parseInt(u.query.seqnr) + 1)
...
const parse = (query, isReturn) => (html) => {
	...
	.map((row, journeyI) => {
		const nextStep = nextStepLink(query, $, row)
		...
		.reduce(parseLegs(query, isReturn, $), {
			...
			journey: {
				type: 'journey',
				id: (isReturn ? 'return' : 'outbound') + '-' + journeyI, // todo
				legs: []
			}
		})
		.journey
