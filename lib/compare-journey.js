...
const compareJourney = (outbound, returning, j, isReturn) => {
	...
	for (let i = 0; i < l; i++) {
		...
		if (!isRoughlyEqual(3 * min, +new Date(qLeg.departure), +new Date(jLeg.departure))) {
			debug('leg', i, 'non-matching departures', qLeg, jLeg)
			return false
		}
		if (!isRoughlyEqual(3 * min, +new Date(qLeg.arrival), +new Date(jLeg.arrival))) {
