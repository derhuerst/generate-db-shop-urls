...
const compareJourney = (q, j, isReturn) => {
	...
	for (let i = 0; i < l; i++) {
		...
		if (!(
			isRoughlyEqual(3 * min, +new Date(leg.departure), +new Date(jLeg.departure))
			&& isRoughlyEqual(3 * min, +new Date(leg.arrival), +new Date(jLeg.arrival))
		)) return false
		...
		if (!jLeg.lines.find((l) => {
			...
			const qName = slugg(leg.line.name.replace(/\s+/, ''))
			return jName === qName
		})) return false
	}
	...
	if (price.amount && (j.price.amount || j.discount.amount)) {
		...
		// todo: it seems like the HAFAS mobile API returns cheaper prices
		if (jPrice > price.amount) return false
	}
